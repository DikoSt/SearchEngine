...
void SaveTestJsonFile(const std::string &fileName,
                      const nlohmann::json &jFileContent) {
...
TEST(converterJSON, PutAnswers) {
    ...
    ASSERT_EQ(
            "docID",
            readJsonAnswer["answers"]["request001"]["relevance"][0].begin().key());
    //  ASSERT_EQ("rank",
    //  readJsonAnswer["answers"]["request001"]["relevance"][0].back().key());
    // TODO подумать и доделать. Надо ли так глубоко проверять, может просто
    // сделать чтение по всем полям
    ...
    for (auto item = readJsonAnswer["answers"].begin();
         item != readJsonAnswer["answers"].end(); ++item, ++numberRequest) {
        ASSERT_EQ("request" +
                  std::string(3 - std::to_string(numberRequest).size(), '0') +
